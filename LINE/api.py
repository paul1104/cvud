...
class LineApi(object):
    ...
    def qrLogin(self, keepLoggedIn=True, systemName=None, appName=None, showQr=False, returnCallbackUrl=None):
        if systemName is None:
            systemName=self.server.SYSTEM_NAME
        if appName is None:
        ...
        self._client = LineSession(self.server.LINE_HOST_DOMAIN, self.server.Headers, self.server.LINE_AUTH_QUERY_PATH).Talk(isopen=False)
        qrCode = self._client.getAuthQrcode(keepLoggedIn, systemName, returnCallbackUrl)
